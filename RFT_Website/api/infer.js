...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // RunPod wraps the vLLM OpenAI-compatible response inside data.output
    const output = data.output;
    const rawOutput = output?.choices?.[0]?.message?.content
      ?? output?.text
      ?? (typeof output === 'string' ? output : JSON.stringify(output));

    const tokenCount = output?.usage?.completion_tokens ?? null;
